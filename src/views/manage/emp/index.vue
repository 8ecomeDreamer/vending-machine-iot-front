  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="empList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否启用" align="center" prop="status" >
          <template #default="scope">{{ scope.row.status === 1 ? '启用' : scope.row.status === 0 ? '停用' : '未知' }}</template>
      </el-table-column>
...
<!--        </el-form-item>-->
        <el-form-item label="区域名称" prop="regionName">
          <el-select v-model="form.regionId" clearable placeholder="请选择区域" @change="handleRegionChange" style="width: 100%">
            <el-option
...
const data = reactive({
  ...
  rules: {
    ...
    status: [
      { required: true, message: "是否启用：1启用 0停用不能为空", trigger: "change" }
    ],
    createTime: [
...
function handleRegionChange(regionId) {
  if (regionId) {
    const selectedRegion = regionList.value.find(item => item.id === regionId);
    form.value.regionName = selectedRegion ? selectedRegion.regionName : null;
  } else {
